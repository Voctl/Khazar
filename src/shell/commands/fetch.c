...
static U0 fetch(U0) {
  ...
  for (i = 0; i < LOGO_LINES; i++) {
    set_char_w_color(SEP_CHAR, COLOR_DARK_GREY,
                     get_offset(info_col - 2, start_row + i));
  }
  ...
  write_at((STR8_C) "MiB", COLOR_DARK_GREY,
           info_col + 9 + dec_len(total_mb) + 1, row + 4);

  write_at((STR8_C) "Free", COLOR_DARK_GREY, info_col, row + 5);
  write_num_at(free_mb, COLOR_WHITE, info_col + 9, row + 5);
  write_at((STR8_C) "MiB", COLOR_DARK_GREY, info_col + 9 + dec_len(free_mb) + 1,
           row + 5);
